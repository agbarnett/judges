# 2_model_SMR.R
# Model the standardised mortality ratio for judges
# Can use both weighted and unweighted data
# also makes a nice plot
# March 2022
source('2_make_Bayes_SMR.R') # makes the WinBUGS models
library(dplyr)
library(stringr)
library(splines)
library(ggplot2)
...
# load the data
if(weighted==FALSE){infile = 'data/SMR.RData' }
if(weighted==TRUE){infile = 'data/SMR.weighted.RData' }
load(infile) # from 1_SMR.R or weighted version
if(weighted==FALSE){
...
# store DIC
dic.row = data.frame(Model=paste('Non-linear df=', df, sep=''), 
...
if(weighted==FALSE){outfile = paste('figures/SMR.nonlinear.', df ,'df.jpg', sep='')}
if(weighted==TRUE){outfile = paste('figures/SMR.nonlinear.', df ,'df.weighted.jpg', sep='')}
...
# compare DIC
dic.row = data.frame( Model='Linear', 
             pD=bugs.linear$pD, DIC= bugs.linear$DIC)
dic.tab = rbind(dic.tab, dic.row)
dic.tab
# store dic and bugs results
if(weighted==TRUE){outfile = paste('results/DIC.weighted.RData', sep='')}
if(weighted==FALSE){outfile = paste('results/DIC.RData', sep='')}
...
if(weighted==FALSE){outfile = paste('figures/SMR.linear.jpg', sep='')}
if(weighted==TRUE){outfile = paste('figures/SMR.linear.weighted.jpg', sep='')}
...
# nice plot
to_plot = filter(results, df==4) # based on DIC
best.plot = ggplot(data=to_plot, aes(x=year, y=SMR, ymin=lower, ymax=upper)) + 
  geom_hline(yintercept = 1, lty=1, col='dark red')+ # reference line at SMR = 1
  geom_line(size=0.2, col=grey(0.4))+
  geom_point(col=grey(0.4), size=0.9)+
  geom_line(aes(x=year, y=fitted), col='blue', size=1.05)+
  ...
  xlab('Year')+
  ylab('Standardised mortality ratio')+
  coord_cartesian(ylim=c(0,4)) + # because of very high SMRs in early years
  theme_bw()+
  theme(panel.grid.minor = element_blank())
outfile = 'figures/SMR.best.jpg'
jpeg(outfile, width=6, height=4, units='in', res=500, quality = 100)
print(best.plot)
dev.off()
